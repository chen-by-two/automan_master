...
if __name__ == '__main__':
    ...
    #更新用例
    #importCase.impotTestCase("/Users/fuzhangsong/PycharmProjects/automan/TestFile/Fzs/newCleanCenterTestCase.xlsx","newCleanCenter").readExcel()
    importCase.impotTestCase("/Users/fuzhangsong/PycharmProjects/automan/TestFile/Fzs/coinServiceKwTestCase.xlsx","coinService").kwReadExcel()
    #执行用例
    repName = str(datetime.now().strftime("%Y%m%d%H%M%S"))
    alluredir = "report/" + repName
    pytest.main(["-s", "testCase/coinService", "-q", "--alluredir", alluredir])
    #生成可视化报告
