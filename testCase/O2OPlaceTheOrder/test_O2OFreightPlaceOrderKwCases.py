...
class Test_O2OFreightPlaceOrder:

	@allure.feature("O2O-发放运费券-登录运营中心后台")
	@allure.severity("blocker")
	def test_O2O_FreightPlaceOrder_001(self):
		# 读取yaml文件
		...
		# 发放运费券-调取接口
		VoucherCreate = bkw.O2OPlaceTheOrderApi_VoucherCreate({"header":{"Content-Type": "application/json;charsetUTF-8","Authorization":"Bearer "+token},"parma":{},"data":{
    "memberId": memberId,
    "schemeId": "20220706095111937161",
    "requestNo": NowTime
},"env":env},"dict")
		# 提取发放运费券errno
		VoucherCreateerrno = ckw.CommonKeyWord().Json_GetJsonValue(VoucherCreate,[["errno"]])
		# 打印结果到控制台
		ckw.CommonKeyWord().Print_ToControl("VoucherCreateerrno:",VoucherCreateerrno)
		# 打印结果到日志
		ckw.CommonKeyWord().Print_ToLog("VoucherCreateerrno:",VoucherCreateerrno)
		# 读取yaml里成功返回断言
		success_errno = ckw.CommonKeyWord().Yaml_GetByKey(yamlFileinfo,"success_errno")
		# 断言对比字典
		assert ckw.CommonKeyWord().Assert_ObjAndObj(VoucherCreateerrno,success_errno)
		# 强制等待
		ckw.CommonKeyWord().Time_Sleep("1.0")

	@allure.feature("O2O-获取用户token")
	@allure.severity("blocker")
	def test_O2O_FreightPlaceOrder_002(self):
		# 读取yaml文件
	...
	@allure.severity("blocker")
	def test_O2O_FreightPlaceOrder_003(self):
		# 读取yaml文件
 ...
 "location": "106.627393,29.712571"
	...
	@allure.severity("blocker")
	def test_O2O_FreightPlaceOrder_004(self):
		# 读取yaml文件
		yamlFiletoken = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/token.yaml")
		# 读取yaml文件
		yamlFileinfo = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/O2OInfo.yaml")
 ...
 "latitude": "29.712571"
	...
	@allure.severity("blocker")
	def test_O2O_FreightPlaceOrder_005(self):
		# 读取yaml文件
		yamlFiletoken = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/token.yaml")
		# 读取yaml文件
		yamlFileinfo = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/O2OInfo.yaml")
 ...
 "latitude": "31.971861"
	...
	@allure.severity("blocker")
	def test_O2O_FreightPlaceOrder_006(self):
		# 读取yaml文件
		yamlFiletoken = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/token.yaml")
		# 读取yaml文件
		yamlFileinfo = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/O2OInfo.yaml")
 ...
 "eshop": [{
  ...
  "deliveryMethods": []
	...
	@allure.severity("blocker")
	def test_O2O_FreightPlaceOrder_007(self):
		# 读取yaml文件
		yamlFiletoken = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/token.yaml")
		# 读取yaml文件
		yamlFileinfo = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/O2OInfo.yaml")
 ...
 "requestNo":requestNo
	...
	@allure.severity("blocker")
	def test_O2O_FreightPlaceOrder_008(self):
		# 读取yaml文件
		yamlFiletoken = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/token.yaml")
		# 读取yaml文件
		yamlFileinfo = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/O2OInfo.yaml")
