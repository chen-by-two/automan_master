...
class Test_O2OPlaceTheOrder:
	...
	@allure.feature("O2O-解析门店二维码链接")
	@allure.severity("blocker")
	def test_O2O_WxJssdk_002(self):
		# 读取yaml文件
		...
		# 解析门店二维码链接-调取接口
		WxJssdk = bkw.O2OPlaceTheOrderApi_ApiWxJssdk({"header":{"Content-Type": "application/json;charsetUTF-8","Authorization":"Bearer "+token},"parma":{},"data":{
 "url": "https://wx.uniondrug.net/v2/open?toshop&merchantId590&storeMerchantId4711"
},"env":env},"dict")
		# 提取解析门店二维码链接errno
		WxJssdkerrno = ckw.CommonKeyWord().Json_GetJsonValue(WxJssdk,[["errno"]])
		# 打印结果到控制台
		ckw.CommonKeyWord().Print_ToControl("WxJssdkerrno:",WxJssdkerrno)
		# 打印结果到日志
		ckw.CommonKeyWord().Print_ToLog("WxJssdkerrno:",WxJssdkerrno)
		# 读取yaml里成功返回断言
		...
		# 断言对比字典
		assert ckw.CommonKeyWord().Assert_ObjAndObj(WxJssdkerrno,success_errno)
		# 强制等待
		ckw.CommonKeyWord().Time_Sleep("1.0")

	@allure.feature("O2O-获取连锁门店信息")
	@allure.severity("blocker")
	def test_O2O_GetPartnerStores_003(self):
		# 读取yaml文件
		yamlFiletoken = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/token.yaml")
		# 读取yaml文件
		yamlFileinfo = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/O2OInfo.yaml")
		...
		env = ckw.CommonKeyWord().Yaml_GetByKey(yamlFileinfo,"env")
		# 获取连锁门店信息-调取接口
		GetPartnerStores = bkw.O2OPlaceTheOrderApi_ApiSweepGetPartnerStores({"header":{"Content-Type": "application/json;charsetUTF-8","Authorization":"Bearer "+token},"parma":{},"data":{
 "merchantId": "590",
 "storeMerchantId": "4711"
},"env":env},"dict")
		# 提取获取连锁门店信息errno
		GetPartnerStoreserrno = ckw.CommonKeyWord().Json_GetJsonValue(GetPartnerStores,[["errno"]])
		# 打印结果到控制台
		ckw.CommonKeyWord().Print_ToControl("GetPartnerStoreserrno:",GetPartnerStoreserrno)
		# 打印结果到日志
		ckw.CommonKeyWord().Print_ToLog("GetPartnerStoreserrno:",GetPartnerStoreserrno)
		# 读取yaml里成功返回断言
		success_errno = ckw.CommonKeyWord().Yaml_GetByKey(yamlFileinfo,"success_errno")
		# 打印结果到日志
		ckw.CommonKeyWord().Print_ToLog("成功返回断言结果：",success_errno)
		# 断言对比字典
		assert ckw.CommonKeyWord().Assert_ObjAndObj(GetPartnerStoreserrno,success_errno)
		# 强制等待
		ckw.CommonKeyWord().Time_Sleep("1.0")
 ...
 "location": "106.627393,29.712571"
	...
	@allure.severity("blocker")
	def test_O2O_CreatePreviewOrder_004(self):
		# 读取yaml文件
		yamlFiletoken = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/token.yaml")
		# 读取yaml文件
		yamlFileinfo = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/O2OInfo.yaml")
 ...
 "latitude": "31.971933"
	...
	@allure.severity("blocker")
	def test_O2O_CreateOrder_006(self):
		# 读取yaml文件
		yamlFiletoken = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/token.yaml")
		# 读取yaml文件
		yamlFileinfo = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/O2OInfo.yaml")
 ...
 "requestNo":requestNo
	...
	@allure.severity("blocker")
	def test_O2O_NotifyPayment_007(self):
		# 读取yaml文件
		yamlFiletoken = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/token.yaml")
		# 读取yaml文件
		yamlFileinfo = ckw.CommonKeyWord().Yaml_Read("./TestFile/O2OPlaceTheOrder/O2OInfo.yaml")
