...
# 读取商品skuNos
skuNos = ckw.CommonKeyWord().Yaml_GetByKey(yamlFile, "skuNo3")
# 读取yaml文件渠道
...
class Test_unifyOrderintegralselfPaid:
    ...
    def test_drug_login(self):
        ...
        bkw.authApi_makeLoginCaptcha({"header": {"Content-Type": "application/json"},
                                     ...
                                     "dict")
        # 打印渠道到控制台
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例001：公众号登陆")
        # 公众号登录
    ...
    def test_drug_confirm_judgeSkuNo(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：","用例002: 统一交易商品详情页屏蔽分享按钮")
    ...
    def test_drug_unifycart_cart_add(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例003：统一交易加入购物车")
    ...
    def test_drug_unifycart_cart_count(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例004：统一交易购物车数量")
    ...
    def test_drug_unifyAdress_default(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例005：统一交易默认地址")
    ...
    def test_drug_unifyCartQuery(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例006：统一交易购车查询")
    ...
    def test_drug_unifyproduct_buyLimit(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例007：统一交易购买限制")
    ...
    def test_drug_unifyorder_confirm_shopping_create (self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例008 ：统一交易购物车创建订单信息")
        # 获取时间戳
        timeStamp = ckw.CommonKeyWord().Time_UnixTimestamp()
        # 等一秒
        ckw.CommonKeyWord().Time_Sleep("1.0")
    ...
    def test_drug_unifyorder_confirm_detail(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例009：获取confirm详情")
    ...
    def test_drug_confirm_userRule_query(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例010：统一交易查询规则")
    ...
    def test_drug_query_multi(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例011：统一交易获取用户可用权益列表接口")
    ...
    def test_drug_member_coupon_query(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例012：统一交易用户优惠券列表查询")
    ...
    def test_drug_confirm_userRole_containsIdentity(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例013：统一交易校验身份")
    ...
    def test_drug_resourceChange(self):
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例014：订单详情页选择资源")
        # 切换使用权益
        resourceChangeData = bkw.unifyorderApi_UnifyorderConfirmResouceChange(
            {"header": {"Content-Type": "application/json;charsetUTF-8", "Authorization": "Bearer " + token},
             "parma": {}, "data": {
                "resourceIdList": ["7d7d6f72b54a4e3881cd"],
                "resourceType": 2,
                "balanceTimes": None,
                "oneTimeValue": 0,
                "balanceValue": 163.4,
                "discountValue": 0.9,
                "rsourceName": "zdh优惠权益百分之90",
                "resourceDescription": "zdh优惠权益百分之90",
                "resourceCutOffDate": "2025-02-21 23:59:59",
                "nominalValue": 0,
                "type": 5,
                "checked": True,
                "confirmNo": confirmNo,
    ...
    def test_drug_order_create(self):
        # 打印渠道到控制台
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例015：统一交易创建订单接口")
    ...
    def test_drug_cashier_payModes(self):
        # 打印渠道到控制台
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例016：统一交易选择支付方式接口")
    ...
    def test_drug_cashier_create(self):
        # 打印渠道到控制台
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例017：统一交易创建支付流水")
    ...
    def test_drug_cashier_query(self):
            # 打印渠道到控制台
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例018：统一交易获取交易流水信息")
    ...
    def test_drug_notify_payment(self):
            # 打印渠道到控制台
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例019：统一交易支付回调")
        ...
        NotifyPaymentData = bkw.unifyOrderCenterApi_UnifyorderNotifyPaymentData(
            ...
            "totalFee": "0.01",
            "cashFee": "0.01",
            "transactionId": "2022072630074851956",
            "outTradeNo": tradeNo,
            "timeEnd": "2022-07-26 10:43:20",
            "refundId": None,
            "outRefundNo": None,
            "tradeStatus": None,
            "refundAmount": None,
    ...
    def test_drug_orderMainQuery(self):
        # 打印渠道到控制台
        ckw.CommonKeyWord().Print_ToControl("开始打印：", "用例020：统一交易主订单查询")
